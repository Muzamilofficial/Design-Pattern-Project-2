...
namespace HMS.Resources.Forms
{
    class GeneralOPDPatient:GeneralPatient
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("insert into GeneralOPDPatient(Password,Name,FatherName,Disease,Age,Gender,ContactNumber)values('" + _Pswd + "','" + _name + "','" + _fName + "','" + _disease + "','" + _age + "','" + _gender + "','" + _cnum + "')", con.Open());
            cmd.ExecuteNonQuery();
            con.Close();
